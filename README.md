...
- **超分替换（OptiScaler）**：开启时把代理 `dxgi.dll`、DLSS 运行库与 `OptiScaler.ini`
  写进游戏目录，目标文件缺失或被改动时会按安装目录里那一份自动补写；
  关闭时回收这些文件，**游戏正在运行、DLL 被占用删不掉时保留部署记录，退出后继续重试**；
  切换 DLSS 版本只保留当前模式需要的组件：切到 DLSS 4 会删掉
  `nvngx_dlssnr.dll` 与 `nvngx.dll_dlssnr.dll`（文件本来就不存在则什么都不做），
  切到 DLSS 5 是超集、只补写不额外删除。删除前一律核对文件身份
  （部署记录哈希 / 与安装目录副本一致 / 代理内含 OptiScaler 标识），
  同名但不是本程序放进去的文件不会被动。
