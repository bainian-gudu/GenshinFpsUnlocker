...
- **超分组件回收**：程序不向游戏目录写入代理 / 渲染组件，安装包里也没有 `upscaler\`
  目录。定位到游戏路径后按历史部署标记清理早期版本放进去的 `dxgi.dll`、
  `OptiScaler.ini` 与运行库，**游戏运行时 DLL 被占用则退出后继续重试**；没有部署标记
  时逐个核对文件内容身份，不是本程序放进去的同名文件不动。
