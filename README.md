...
- **超分替换（已移除）**：不再向游戏目录写入任何代理 / 渲染组件，安装包里也不再有
  `upscaler\` 目录。仅保留一次性的历史回收：定位到游戏路径后按旧版本的部署标记清理
  之前放进去的 `dxgi.dll`、`OptiScaler.ini` 与运行库，**游戏运行时 DLL 被占用则退出后
  继续重试**；没有部署标记时逐个核对文件内容身份，不是本程序放进去的同名文件不动。
