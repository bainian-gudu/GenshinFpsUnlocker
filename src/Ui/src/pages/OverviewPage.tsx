    ...
    <>
      ...
      <motion.div className="overview-controls" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.12 }}>
        ...
        <div className="overview-right-stack">
          ...
          <section className="control-panel upscaler-download-panel" aria-label="超分辨率组件下载"><div className="panel-heading"><h2><Download size={17} strokeWidth={1.7} />组件说明与下载</h2></div><p>需要 NVIDIA RTX 显卡、兼容驱动、OptiScaler.dll 和官方 DLSS Runtime。代理组件与帧率解锁、反虚化注入独立运行。</p>{native && <button className="button button-secondary upscaler-download-button" type="button" onClick={() => void downloadDlssRuntime()} disabled={downloadingDlss || upscaler.dlssRuntimePresent}>{downloadingDlss ? <LoaderCircle size={14} className="spin" /> : <Download size={14} />}{downloadingDlss ? '下载中…' : upscaler.dlssRuntimePresent ? 'DLSS Runtime 已存在' : '下载 DLSS Runtime'}</button>}</section>
        </div>
      </motion.div>
