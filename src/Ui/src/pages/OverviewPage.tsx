      ...
      <motion.div className="overview-controls" initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4, delay: 0.12 }}>
        <div className="overview-left-stack">
          ...
          <section className="control-panel upscaler-card" aria-label="超分辨率替换">
            ...
            <div className="upscaler-card-footer"><p>需要 OptiScaler.dll 与 DLSS Runtime；代理加载后可在游戏内按 Insert 查看“FSR 2.X → DLSS”和持续增长的 FrameCount。</p>{native && <button className="button button-secondary upscaler-download-button" type="button" onClick={() => void downloadDlssRuntime()} disabled={downloadingDlss || upscaler.dlssRuntimePresent}>{downloadingDlss ? <LoaderCircle size={13} className="spin" /> : <Download size={13} />}{downloadingDlss ? '下载中…' : upscaler.dlssRuntimePresent ? 'Runtime 已存在' : '下载 Runtime'}</button>}</div>
          </section>
        </div>
