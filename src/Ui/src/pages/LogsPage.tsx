...
const LogRow = memo(function LogRow({ entry }: { entry: LogEntry }) {
  return <tr><td><time dateTime={entry.timestamp}>{formatTime(entry.timestamp)}</time></td><td><span className={`log-level level-${entry.level.toLowerCase()}`}>{entry.level.toUpperCase()}</span></td><td>{entry.message}</td></tr>;
});
...
export function LogsPage({ logs, onClear, onExport, onOpenFolder, isNative }: { logs: LogEntry[]; onClear: () => void; onExport: () => void; onOpenFolder?: () => void; isNative?: boolean }) {
  ...
  return <>
    ...
    <section className="log-workspace">
      ...
      <div className="log-table-container"><table className="log-table"><thead><tr><th>时间</th><th>级别</th><th>事件详情</th></tr></thead><tbody>{filtered.map((entry) => <LogRow key={entry.id} entry={entry} />)}</tbody></table>
