...
import type { AutostartState, UpscalerState } from '../lib/native';
...
export function SettingsPage({ config, updateConfig, onPath, onExport, onImport, onReset, onUninstall, busy, isNative, isElevated, onRestartElevated, elevating, upscaler, autostart }: {
  ...
  isElevated?: boolean; onRestartElevated?: () => void; elevating?: boolean; upscaler: UpscalerState; autostart: AutostartState;
}) {
    ...
    <div role="tabpanel" id={`settings-${tab}`} aria-labelledby={`tab-${tab}`} className="settings-tab-content" key={tab}>
      {tab === 'game' && <>
        ...
        <section className="control-panel settings-control settings-upscaler"><div className="panel-heading"><h2><Sparkles size={18} />超分辨率替换</h2><span className="feature-badge">实验组件</span></div>
          ...
          <p className="settings-small-note"><Info size={14} />替换功能与帧率解锁、反虚化注入独立运行。关闭后会自动回收游戏目录里的代理组件（游戏正在运行时，退出后完成清理）；切换 DLSS 版本会清掉当前版本用不到的组件，需重启游戏才能生效。</p>
        </section>
