...
import { BRAND_NAME, BRAND_SUB, PROJECT_URL } from '../lib/config';
...
export function AboutPage({ onSafety, version = '1.0.0', isNative, onUninstall }: { onSafety: () => void; version?: string; isNative?: boolean; onUninstall?: () => void }) {
  return <>
    ...
    <div className="about-disclaimer"><ShieldCheck size={18} /><p>本项目为个人自用的第三方工具，代码与文档主要由 AI 生成；与米哈游 / HoYoverse 无关联，未获授权或背书。「原神」「崩坏：星穹铁道」及其角色、图标、场景素材版权均归米哈游所有。使用风险自负。<button className="text-button" onClick={onSafety}>查看用户协议</button></p></div>
