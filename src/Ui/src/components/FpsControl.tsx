 ...
 * 传 `lock` 时（星穹铁道：走注册表、只支持 120 帧）不提供任何自定义入口，
 * 只显示固定值与说明，避免界面给出游戏其实不支持的档位。
 */
import { Gauge, Lock, Monitor, Sparkles } from 'lucide-react';
...
export function FpsControl({ value, enabled, masterEnabled, onChange, onToggle, lock }: {
  value: number;
  enabled: boolean;
  masterEnabled: boolean;
  onChange: (value: number) => void;
  onToggle: (value: boolean) => void;
  /** 该游戏的帧率锁定信息；有值时整个面板只读。 */
  lock?: GameMeta['fpsLock'];
}) {
  const [draft, setDraft] = useState(String(value));
  const [error, setError] = useState('');
  useEffect(() => { setDraft(String(value)); setError(''); }, [value]);
  const inactive = !enabled || !masterEnabled;
  const locked = lock?.value ?? 0;
  ...
  return (
    <section className="control-panel fps-panel" aria-label="帧率解锁设置">
      <div className="panel-heading">
        <h2><Gauge size={18} strokeWidth={1.7} />帧率解锁</h2>
        <div className="panel-toggle">{lock && <span className="module-chip" title="通过注册表写入，不注入游戏进程">注册表</span>}<span>{enabled ? '已开启' : '已关闭'}</span><Toggle label="启用帧率解锁" checked={enabled} onChange={onToggle} /></div>
      </div>
      {lock ? (
        <div className="fps-locked">
          <div className="fps-value-row">
            <div>
              <span className="field-caption">目标帧率</span>
              <div className="fps-number-wrap">
                <span className="fps-number-static" aria-label={`目标帧率固定为 ${locked} FPS`}>{locked}</span>
                <span className="fps-unit">FPS</span>
                <span className="recommended"><Lock size={11} />固定</span>
              </div>
            </div>
            <div className="fps-monitor-hint"><Monitor size={17} strokeWidth={1.5} /><span>游戏内帧率档位<br />由注册表改写</span></div>
          </div>
          <ul className="fps-locked-notes">{lock.notes.map((note) => <li key={note}>{note}</li>)}</ul>
        </div>
      ) : (
      ...
      <p id="fps-feedback" className={`fps-feedback ${error ? 'field-error' : ''}`} role={error ? 'alert' : undefined}>
        {error || (!masterEnabled ? '总开关已关闭，请前往设置启用解锁服务。'
          : !enabled ? (lock ? '帧率解锁已暂停，不再检查或写入注册表；游戏沿用注册表里已有的设置。' : '帧率解锁已暂停，游戏恢复自身档位；你的目标帧率会被保留。')
          : lock ? '已启用：桌面版会在开启时检查注册表，必要时写入 120 FPS。'
          : value > 240 ? '高帧率会增加设备负载，请根据屏幕刷新率与性能选择。'
          : value < 30 ? '当前上限较低，可能影响游戏流畅度，建议从 60 FPS 开始。'
          : '拖动滑块或点击数值自定义，设置将自动保存。')}
      </p>
    </section>
  );
}
